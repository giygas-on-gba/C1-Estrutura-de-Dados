...
int main(int argc, char *argv[]) {
	...
	do {
		...
		////Se não tivesse, a inserção seria à esquerda
		...
		d->size++; // Incrementa o num. de nós
		count++;
		printf("%i ", p->info);
    ...
    int soma = 0;
    while (temp != NULL) {
        soma += temp->info; // Faz o somatório de todos os itens
        temp = temp->next;
    }
    float media = (float)soma / d->size; // Media aritmetica é calculada aqui
    printf("\nValor medio:\n");
	printf("%.2f\n", media);
